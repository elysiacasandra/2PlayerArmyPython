...
"""
__author__ = "Maria Garcia de la Banda for the base"+" 29694736 student for"
...
class CircularQueue(Queue[T]):
    ...
    def __str__(self) -> str:
        """Computes a string from the queue items - bottom to top."""
        length = len(self)
        if length > 0:
            output = str(self.array[self.front])
            for i in range(self.front + 1, 1, self.rear-1):
